...
public class YamlMojoReader {
    ...
    private Plugin convertToMojo(Map<String, Object> mojo) {
        ...
        throw new RuntimeException("unknown type in your mojo definition. In " + String.valueOf(mojo.get("name"))
                + "expected keys 'plugin' or 'ant'. ");
    }
    ...
    public Lifecycle readFromString(String processFileContent) throws Exception {
        ...
        if ("none".equals(lifecycle.packaging)) {
            ...
            lifecycle.standaloneMojo.className = WordUtils.capitalize(lifecycle.standaloneMojo.name.replace("-", " "))
                    .replace(" ", "");
